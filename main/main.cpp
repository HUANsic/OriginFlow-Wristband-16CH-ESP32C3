...
const sensor_if_t *init_ad7689_1() {
  ...
  uint16_t dummy;
  for (int i = 0; i < 2; i++) {
    _ad7689_if->read_channel_data(_ad7689_if, 0, &dummy);
...
void sensor_read_task(void *param) {
  ...
  while (1) {
    ...
    ESP_LOGI(TAG, "start_time : %" PRId64, start_time);
#if ENABLE_IMU
    lsm6ds3tr_c_xl_flag_data_ready_get(&dev_ctx, &drdy);
    last_time = get_timestamp();
    ESP_LOGI(TAG, "1 last_time - start_time: %" PRId64, last_time - start_time);
    if (drdy) {
      lsm6ds3tr_c_acceleration_raw_get(NULL, outaccel);
      last_time = get_timestamp();
      ESP_LOGI(TAG, "2 last_time - start_time: %" PRId64, last_time - start_time);
      ...
      // last_time = get_timestamp();
      // ESP_LOGI(TAG, "3 last_time - start_time: %" PRId64, last_time - start_time);
    }
    lsm6ds3tr_c_gy_flag_data_ready_get(&dev_ctx, &drdy);
    last_time = get_timestamp();
    ESP_LOGI(TAG, "4 last_time - start_time: %" PRId64, last_time - start_time);

    if (drdy) {
      lsm6ds3tr_c_angular_rate_raw_get(NULL, outgyro);
      last_time = get_timestamp();
      ESP_LOGI(TAG, "5 last_time - start_time: %" PRId64, last_time - start_time);
    }
    // lsm6ds3tr_c_temp_flag_data_ready_get(&dev_ctx, &drdy);
    // last_time = get_timestamp();
    // ESP_LOGI(TAG, "6 last_time - start_time: %" PRId64, last_time - start_time);
    // if (drdy) {
    ...
    //   last_time = get_timestamp();
    //   ESP_LOGI(TAG, "7 last_time - start_time: %" PRId64, last_time - start_time);
    // }
    data_raw_temperature = 0;
    // lis3mdl_temperature_raw_get(&mag_ctx, &data_raw_temperature);
    // last_time = get_timestamp();
    // ESP_LOGI(TAG, "8 last_time - start_time: %" PRId64, last_time - start_time);
    ...
    ESP_LOGI(TAG, "last_time %" PRId64, "endtime: %" PRId64, start_time, end_time);
    ESP_LOGI(TAG, "end_time - last_time: %" PRId64, end_time - start_time);
    ...
    for (int idx = 0; idx < 2; idx++) {
      for (int i = 0; i < 8; i++) {
        ...
        if (channel >= 0 && channel < 8) {
          ...
          voltage[idx][channel] = ((values[idx][channel] - AD7689_BASE_V) / 65536.0) * 2.5;
        }
        vTaskDelay(1 / portTICK_PERIOD_MS);
      }
    }
