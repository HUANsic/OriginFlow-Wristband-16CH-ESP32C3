...
static void start_advertising(void) {
  ...
  rsp_fields.sol_uuids128 = &ble_tx_rx_svc_uuid;
  ...
  switch (event->type) {
    ...
    case BLE_GAP_EVENT_CONNECT:
      ...
      if (event->connect.status == 0) {
        ...
        struct ble_gap_upd_params params = {
            .itvl_min = desc.conn_itvl, .itvl_max = desc.conn_itvl, .latency = 3, .supervision_timeout = desc.supervision_timeout};
    ...
    case BLE_GAP_EVENT_SUBSCRIBE:
      ...
      ESP_LOGI(TAG,
               ...
               event->subscribe.conn_handle, event->subscribe.attr_handle, event->subscribe.reason, event->subscribe.prev_notify, event->subscribe.cur_notify,
